...
## 结合隐式和显式融入规则的方法
在训练时在训练过程中使用隐式融入规则方法增强模型的推理能力，在推理 过程中采用显式融入规则方法确保答案的完整性与准确性。

代码运行步骤：1.进行知识图谱嵌入模型的训练，将规则信息融入到嵌入模型中，模型参数存储在best_valid.model中 2.利用训练好的模型对重写后的查询集进行多跳推理，结果保存在results中
